...
lives" and "how a connection is configured." Local-first by design: this is
one file on the machine running the app — no database server to install,
configure, or keep alive, and it works with zero internet connection. (This
project briefly ran on hosted Postgres for remote dashboard access — see git
history — but that traded away the offline guarantee for a database with
its own separate lifecycle, so it moved back here.)
"""
